 ...
 * @constructor
 */
 export const gpxParser = function () {
    this.xmlSource = "";
    this.metadata  = {};
    this.waypoints = [];
    this.tracks    = [];
 ...
 */
gpxParser.prototype.parse = function (gpxstring:string) {
    let keepThis = this;
 ...
 */
gpxParser.prototype.getElementValue = function(parent: Element, needle: string):string {
    let elem = parent.querySelector(needle);
    if(elem != null){
        return elem.innerHTML != undefined ? elem.innerHTML : elem.childNodes[0].textContent;
    }
    return '';
};
 ...
 */
gpxParser.prototype.queryDirectSelector = function(parent: Element, needle: string):Element|ChildNode  {
 ...
 * 
 * @return {Distance} An object with total distance and Cumulative distances
 */
gpxParser.prototype.calculDistance = function(points:Point[] ): Distance {
    let distance = {} as Distance;
    let totalDistance = 0;
    let cumulDistance = [];
 ...
 */
gpxParser.prototype.calcDistanceBetween = function (wpt1:Point , wpt2:Point ): number {
    let latlng1 = {} as Point;
    latlng1.lat = wpt1.lat;
    latlng1.lon = wpt1.lon;
    let latlng2 = {} as Point;
 ...
 */
gpxParser.prototype.calcElevation = function (points: Point[]): Elevation {
    var dp = 0,
        dm = 0,
        ret = {} as Elevation;
    ...
    for (var i = 0; i < points.length - 1; i++) {
        ...
        if(rawNextElevation !== null && rawElevation !== null) {
            let diff = rawNextElevation - rawElevation
 ...
 * 
 * @returns {Slope[]} An array of slopes
 */
gpxParser.prototype.calculSlope = function(points:Point[] , cumul:number[] ): number[] {
    let slopes = [];
